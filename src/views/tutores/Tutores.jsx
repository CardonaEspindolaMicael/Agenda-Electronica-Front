...
const Tutores = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await ApiRequests.getCommon("/usuario/tutores");
        setData(response)
      } catch (error) {
        console.log(error);
      }
  ...
  const insertarArchivos = async (obtenerExito) => {
    ...
    try {
      const response = await ApiRequests.postCommon("/usuario/registrarTutores", formData);
      setBorro(true)
    } catch (error) {
  ...
  return (
    <div className="row" style={{ margin: '20px 50px 0 50px' }}>
      ...
      <SearchFilter value={search} onChange={searcher} placeholder='INGRESE EL CARNET TUTOR ....' />
...

          {
            currentItems.map((Usuario) => (
              <tr key={Usuario.ci}>
                ...
                <td>
                  <IrButton
                    navigateTo='/restablecerContraseñaTutor'
                    object={Usuario}
                    identificador="1"
                  />
                </td>
      ...
      <NewButton titulo='Nuevo Tutor' navigateTo='/nuevoUsuarioTutor' />
    </div>
  );
}
